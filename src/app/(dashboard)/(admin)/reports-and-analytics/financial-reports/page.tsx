...
export default function ClosedRegistersPage() {
  ...
  const loadClosedRegisters = async () => {
    ...
    try {
      const response = await fetch(
        `/api/closed-registers?period=${selectedPeriod}`,
      );
  ...
  const summary = {
    ...
    totalSales: registers.reduce(
      (sum, r) => sum + (r.snapshot?.totalSales || 0),
      0,
    ),
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-semibold text-card-foreground">
                        {register.registerName || "Main Register"}
                      </h3>
                  ...
                  <div className="flex items-center gap-3">
                    {register.closeStatus && (
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-medium border ${getStatusColor(register.closeStatus)}`}
                      >
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
                  <div>
                    ...
                    <p className="font-medium text-card-foreground">
                      {formatCurrency(register.openingFund)}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground">Total Sales</p>
                    <p className="font-medium text-card-foreground">
                      {formatCurrency(register.snapshot?.totalSales || 0)}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground">Cash Sales</p>
                    <p className="font-medium text-card-foreground">
                      {formatCurrency(register.snapshot?.cashSales || 0)}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground">
                    ...
                    <p className="font-medium text-card-foreground">
                      {register.snapshot?.transactions || 0}
                    </p>
                  </div>
                </div>

                {/* Cash Details */}
                {register.expectedCash !== undefined &&
                  register.actualCash !== undefined && (
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 p-4 bg-muted rounded-lg">
                      <div>
                        ...
                        <p className="font-medium text-card-foreground">
                          {formatCurrency(register.expectedCash)}
                        </p>
                      </div>
                      <div>
                        <p className="text-xs text-muted-foreground">
