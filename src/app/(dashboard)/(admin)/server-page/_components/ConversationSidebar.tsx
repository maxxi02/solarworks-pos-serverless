        ...
        <div className="flex items-center justify-between">
          ...
          {lastMsg && (
            <span className="text-xs text-slate-500 shrink-0 ml-2">
              {formatDistanceToNowStrict(new Date(lastMsg.sentAt), {
                addSuffix: false,
              })}
            </span>
          )}
        ...
        {lastMsg && (
          <p
            className={cn(
              ...
              unread > 0 ? "text-slate-300" : "text-slate-500",
            )}
          >
            {conv.type === "group" && `${lastMsg.senderName}: `}
            {lastMsg.hasAttachment && !lastMsg.content
              ? "📎 Attachment"
              : lastMsg.content}
          </p>
        )}
