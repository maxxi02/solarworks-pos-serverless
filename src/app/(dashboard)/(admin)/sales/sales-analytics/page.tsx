...
export default function SalesAnalyticsPage() {
  ...
  // Socket connection state from unified provider
      ...
      <div className="min-h-screen bg-background p-6 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">
            Loading sales analytics from database...
          </p>
        </div>
      </div>
    );
  ...
  const topProducts = Array.isArray(salesData?.topProducts)
    ? salesData.topProducts
    : [];
  const paymentMethods = Array.isArray(salesData?.paymentMethods)
    ? salesData.paymentMethods
    : [];
  const recentTransactions = Array.isArray(salesData?.recentTransactions)
    ? salesData.recentTransactions
    : [];
      ...
      <main className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
          <div>
            ...
            {salesData?.dateRange && (
              <p className="text-xs text-muted-foreground mt-1">
                Period: {formatDate(salesData.dateRange.from)} -{" "}
                {formatDate(salesData.dateRange.to)}
              </p>
            )}
          </div>
                            ...
                            >
                              {paymentMethods.map((entry, index) => (
                                <Cell
                                  key={`cell-${index}`}
                                  fill={
                                    COLORS[entry._id as keyof typeof COLORS] ||
                                    COLORS.primary
                                  }
                                />
                              ))}
                          ...
                          >
                            <div className="flex items-center gap-2">
                              <div
                                ...
                                style={{
                                  backgroundColor:
                                    COLORS[method._id as keyof typeof COLORS] ||
                                    COLORS.primary,
                                }}
                              />
