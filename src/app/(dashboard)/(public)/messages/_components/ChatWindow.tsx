...
import type { ConversationWithDetails, Message, OptimisticMessage, MessageAttachment } from "@/types/messaging.types";
...
export function ChatWindow({
    ...
    const handleSend = useCallback((content: string, attachments?: MessageAttachment[]) => {
        sendMessage(content, currentUserName, currentUserImage, attachments);
    }, [sendMessage, currentUserName, currentUserImage]);

    // ── Header display ──────────────────────────────────────────────
    const headerTitle = isGroup ? conversation.groupName : conversation.otherParticipant?.name;
    ...
    const isOtherOnline = !isGroup && conversation.otherParticipant?.isOnline;
        ...
        <div className="flex flex-col h-full overflow-hidden bg-background">
            ...
            <div className="flex items-center gap-3 px-4 py-3 border-b border-border bg-card flex-shrink-0 z-10 animate-in fade-in duration-200">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <p className={cn("text-[11px] text-muted-foreground", isOtherOnline && "text-emerald-500")}>
                        {headerSubtitle()}
                    </p>
                </div>
                    ...
                    <div className="flex flex-col items-center justify-center h-full text-center animate-in zoom-in-95 duration-300">
                        ...
                        <p className="text-sm text-muted-foreground mt-2 max-w-[200px]">
                            {isGroup
                                ...
                                : `This is the beginning of your chat with ${headerTitle}.`}
                        </p>
                    </div>
                ) : (
            ...
            {/* Input */}
            <div className="flex-shrink-0 px-3 pb-3 pt-2 bg-background/80 backdrop-blur-sm border-t border-border">
                <MessageInput
                    onSend={handleSend}
