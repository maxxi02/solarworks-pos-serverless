...
export function ChatDrawer({
  ...
  useEffect(() => {
    ...
    socket.on(historyResultEvent, handleHistoryResult);
    socket.on(receiveEvent, handleReceiveMessage);

    return () => {
      socket.emit(leaveEvent, joinData);
      socket.off(historyResultEvent, handleHistoryResult);
      socket.off(receiveEvent, handleReceiveMessage);
    };
  }, [socket, sessionId, tableId, staffId, staffName]);
      ...
      <div className="flex items-center justify-between p-4 border-b border-border">
        <div>
          <h3 className="font-bold text-foreground text-sm">
            {tableId ? "Table Chat" : "Customer Chat"}
          </h3>
          <p className="text-xs text-muted-foreground">
            {tableId ? "Table" : "Session"}:{" "}
            {(sessionId || tableId || "").slice(0, 8)}...
          </p>
        </div>
        <button
          onClick={onClose}
