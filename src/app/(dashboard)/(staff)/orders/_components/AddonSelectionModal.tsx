...
export function AddonSelectionModal({
  product,
  open,
  onClose,
  onConfirm,
  ...
  // Compute addon total for footer display
  ...
  const missingRequired = groups
    .filter((g) => g.required)
    .filter((g) => !(selections[g.name]?.size));
      ...
      <DialogContent className="max-w-md sm:max-w-lg p-0 gap-0 overflow-hidden">
        ...
        <ScrollArea className="max-h-[50vh]">
          <div className="px-6 py-4 space-y-5">
            {groups.map((group) => {
              const isMissing =
                group.required && !(selections[group.name]?.size);
              return (
                <div key={group.name}>
                  {/* Group Header */}
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    {group.required ? (
                      <Badge
                        variant="destructive"
                        className={`text-[10px] h-4 px-1.5 ${isMissing ? "animate-pulse" : ""}`}
                      >
                        Required
                      </Badge>
                    ) : (
                    ...
                    {group.items.map((item) => {
                      ...
                      return (
                        <button
                          ...
                          className={`
                            flex items-center justify-between w-full px-4 py-2.5 rounded-lg border text-left transition-all
                            ${
                              selected
        ...
        {/* Footer */}
        <DialogFooter className="px-6 py-4 flex-col sm:flex-row gap-3 items-center">
          {/* Quantity */}
