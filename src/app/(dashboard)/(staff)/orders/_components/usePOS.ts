...
export const useCart = (playOrder: () => void, playError: () => void, playSuccess: () => void) => {
  const [cart, setCart] = useState<CartItem[]>([]);

  const subtotal = useMemo(() => cart.reduce((sum, i) => sum + i.price * i.quantity, 0), [cart]);
  const discountTotal = useMemo(
    () => cart.reduce((sum, i) => (i.hasDiscount ? sum + i.price * i.quantity * DISCOUNT_RATE : sum), 0),
    [cart]
  );
  ...
  const applyDiscount = useCallback((data: { ids: string[]; itemIds: string[] }) => {
    setCart(prev => prev.map(item => data.itemIds.includes(item._id) ? { ...item, hasDiscount: true } : item));
    toast.success(`Discount applied to ${data.itemIds.length} item(s)`);
    playSuccess();
  }, [playSuccess]);

  const removeDiscount = useCallback((itemId: string) => {
    setCart(prev => prev.map(item => item._id === itemId ? { ...item, hasDiscount: false } : item));
  ...
  return {
    ...
    addToCart, updateQuantity, removeFromCart, applyDiscount, removeDiscount, clearCart,
  };
};
...
export const useSavedOrders = () => {
  const [savedOrders, setSavedOrders] = useState<SavedOrder[]>(() => {
    try {
      ...
      if (saved) {
        return JSON.parse(saved).map((o: SavedOrder) => ({ ...o, timestamp: new Date(o.timestamp) }));
      }
    } catch {}
    return [];
  });
