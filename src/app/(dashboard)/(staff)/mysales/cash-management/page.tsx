...
export default function CashManagementPage() {
  ...
  const getCashInDrawer = useCallback(
    (s: SessionData | null, cashSales: number, period: string): number => {
      if (!s) return 0;
      const now = new Date();
      ...
      const effectiveStart =
        sessionStart > periodStart ? sessionStart : periodStart;
  ...
  const getPeriodCashOuts = useCallback(
    (s: SessionData | null, period: string): number => {
      if (!s) return 0;
      const now = new Date();
      const todayStart = new Date(
        now.getFullYear(),
  ...
  const drawerBalance = getCashInDrawer(
    session,
    summary.cashSales,
    selectedPeriod,
  );
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">
              Opened{" "}
              {session?.openedAt
                ? new Date(session.openedAt).toLocaleTimeString("en-US", {
                    hour: "2-digit",
                    minute: "2-digit",
                  })
                : "—"}
