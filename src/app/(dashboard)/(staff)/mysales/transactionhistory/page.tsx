...
const History = () => {
  ...
  const fetchTransactions = async () => {
    setLoading(true)
    try {
      const response = await fetch(`/api/payments?page=${currentPage}&limit=50`)
  ...
  const filteredTransactions = transactions.filter(t => {
    ...
    const matchesDate = !dateFilter?.start || !dateFilter?.end
      ? true
      : transactionDate >= new Date(dateFilter.start) && transactionDate <= new Date(dateFilter.end)
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="mb-6 flex justify-between items-center">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">View and manage all your sales transactions from MongoDB</p>
          </div>
                ...
                {currentItems.map(t => {
                  ...
                  return (
                    <tr key={t._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-sm text-gray-500">{formatDate(t.timestamp || t.createdAt)}</td>
          ...
          {totalPages > 1 && (
            <div className="px-4 py-3 border-t border-gray-200 flex items-center justify-between">
              <p className="text-sm text-gray-500">
                Showing {indexOfFirstItem + 1} to {Math.min(indexOfLastItem, sortedTransactions.length)} of {totalTransactions} transactions
              </p>
              <div className="flex gap-2">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4 p-4 bg-gray-50 rounded-lg">
                    <div>
                      ...
                      <p className="text-sm font-medium">{formatDate(selectedTransaction.timestamp || selectedTransaction.createdAt)}</p>
                    </div>
                    <div>
