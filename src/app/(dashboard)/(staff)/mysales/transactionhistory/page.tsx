...
const History = () => {
  ...
  const calculateStats = () => {
    ...
    const base = transactions.filter((t) => {
      ...
      const matchesDate =
        !df?.start || !df?.end
          ? true
          : d >= new Date(df.start) && d <= new Date(df.end);
      return (
        matchesSearch &&
        matchesStatus &&
        matchesPayment &&
    ...
    const qrSales = transactions
      .filter(
        (t) =>
          t.status === "completed" &&
          t.orderType === "qr" &&
          new Date(t.timestamp || t.createdAt) >= todayStart,
      )
  ...
  const getDateFilter = () => {
    ...
    switch (dateRange) {
      ...
      case "week":
        return {
          start: new Date(today.getTime() - 7 * 86400000).toISOString(),
          end: now.toISOString(),
        };
      case "month":
        return {
          start: new Date(today.getTime() - 30 * 86400000).toISOString(),
          end: now.toISOString(),
        };
      case "custom":
        return {
  ...
  const buildReceiptInput = useCallback(
    (transaction: Transaction): ReceiptBuildInput => {
      return {
        orderNumber: transaction.orderNumber,
        customerName: transaction.customerName,
        cashier: transaction.cashier,
        timestamp: new Date(transaction.timestamp || transaction.createdAt),
        orderType: (transaction.orderType === "qr"
          ? "takeaway"
          : transaction.orderType) as "dine-in" | "takeaway",
  ...
  const printFallback = useCallback(
    (transaction: Transaction) => {
      ...
      const discountTotal =
        transaction.discountTotal ?? transaction.discount ?? 0;
      ...
      <div class="mb-1">
        ...
        ${transaction.items
          .map((item) => {
            const dp = item.hasDiscount
              ? item.price * (1 - DISCOUNT_RATE)
              : item.price;
            return `<div class="flex justify-between"><span>${item.name}</span><span>${item.quantity} ${fmt(dp * item.quantity)}</span></div>${item.hasDiscount ? `<div style="font-size:.85em;padding-left:8px;">(20% Senior/PWD) -${fmt(item.price * item.quantity * DISCOUNT_RATE)}</div>` : ""}`;
          })
          .join("")}
      </div>
      <div class="text-center mb-1">${dash}</div>
  ...
  const handlePrint = useCallback(
    async (transaction: Transaction) => {
      ...
      const canUseCompanion =
        isConnected && (companionStatus.usb || companionStatus.bt);
      if (canUseCompanion) {
        try {
      ...
      "Amount Paid":
        t.paymentMethod === "cash"
          ...
          : t.paymentMethod === "split" && t.splitPayment
            ? t.splitPayment.cash + t.splitPayment.gcash
            : t.total,
      Change:
        t.paymentMethod === "cash"
          ? (t.change ?? 0)
          : t.paymentMethod === "split" && t.splitPayment
            ? Math.max(0, t.splitPayment.cash + t.splitPayment.gcash - t.total)
            : 0,
      ...
      <div className="max-w-7xl mx-auto p-6 md:p-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-px rounded-xl overflow-hidden mb-8 border border-border bg-border">
          {[
            ...
            {
              label: "Cash",
              value: fmt(stats.cashSales),
              sub: `${((stats.cashSales / stats.totalSales) * 100 || 0).toFixed(1)}% of total`,
              icon: DollarSign,
            },
            {
              label: "GCash",
              value: fmt(stats.gcashSales),
              sub: `${((stats.gcashSales / stats.totalSales) * 100 || 0).toFixed(1)}% of total`,
              icon: Smartphone,
            },
            {
              label: "Today's QR Sales",
              value: fmt(stats.qrSales),
              sub: "QR orders today",
              icon: QrCode,
            },
                  ...
                  >
                    ...
                    >
                      <CompanionPrintButton
                        ...
                        isPrinting={
                          isPrinting && selectedTransaction?._id === t._id
                        }
                        hasPrinted={false}
              ...
              <div className="p-6 md:p-8">
                    ...
                    { label: "Cashier", value: selectedTransaction.cashier },
                    {
                      ...
                      value:
                        ...
                        (selectedTransaction.tableNumber
                          ? ` — Table ${selectedTransaction.tableNumber}`
                          : ""),
                    },
                    {
                      label: "Payment",
                      value: selectedTransaction.paymentMethod.toUpperCase(),
                    },
                  ].map(({ label, value }) => (
                  ...
                  <div className="border border-border rounded-xl overflow-hidden divide-y divide-border">
                    {selectedTransaction.items.map((item, i) => {
                      ...
                      return (
                        <div
                          key={i}
                          className="flex justify-between items-center px-4 py-3 bg-background"
                        >
                          <div>
                ...
                <div className="border border-border rounded-xl p-4 mb-5 space-y-2 bg-background">
                  <div className="flex justify-between text-sm text-muted-foreground">
                    <span>Subtotal</span>
                    <span>{fmt(selectedTransaction.subtotal)}</span>
                  </div>
                  {(selectedTransaction.discountTotal ??
                  ...
                  <div className="border-t border-border pt-2 mt-2 flex justify-between font-bold text-base text-foreground">
                    <span>Total</span>
                    <span>{fmt(selectedTransaction.total)}</span>
                  </div>
                </div>
                ...
                <div className="border border-border rounded-xl p-4 mb-6 bg-background">
                  ...
                  <div className="space-y-2">
                    {selectedTransaction.paymentMethod === "cash" &&
                      selectedTransaction.amountPaid && (
                        <>
                          <div className="flex justify-between text-sm text-muted-foreground">
                            <span>Received</span>
                            <span>{fmt(selectedTransaction.amountPaid)}</span>
                          </div>
                    ...
                    {selectedTransaction.paymentMethod === "split" &&
                      selectedTransaction.splitPayment && (
                        <>
                          <div className="flex justify-between text-sm text-muted-foreground">
                            <span>Cash</span>
                            <span>
                              {fmt(selectedTransaction.splitPayment.cash)}
                            </span>
                          </div>
                          <div className="flex justify-between text-sm text-muted-foreground">
                            <span>GCash</span>
                            <span>
                              {fmt(selectedTransaction.splitPayment.gcash)}
                            </span>
                          </div>
                          <div className="border-t border-border pt-2 flex justify-between text-sm font-semibold text-foreground">
                            <span>Total Paid</span>
                            <span>
                              {fmt(
                                selectedTransaction.splitPayment.cash +
                                  selectedTransaction.splitPayment.gcash,
                              )}
                            </span>
                          </div>
                          {selectedTransaction.splitPayment.cash +
