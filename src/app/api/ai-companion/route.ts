  ...
  switch (type) {
    ...
    case "all-time-stats": {
      const [
        ...
        paymentsCollection
          .aggregate([
            { $match: { ...(staffId && { cashierId: staffId }) } },
            {
              $group: {
                _id: null,
                totalRevenue: { $sum: "$total" },
    ...
    case "inventory-insights": {
      const [lowStockItems, criticalStockItems, totalProducts, totalValue] =
        await Promise.all([
          inventoryCollection
            .aggregate([
              {
    ...
    case "attendance": {
      ...
      const [recentAttendance, totalStats] = await Promise.all([
      ...
      return {
        ...
        lastClockIn: latest?.clockInTime || "N/A",
        totalHours: totalStats[0]?.totalHours?.toFixed(1) || "0",
      };
    }
...
export async function GET(request: Request) {
  try {
    ...
    switch (type) {
      case "summary":
      case "today-highlights": {
        const [todaySales, yesterdaySales, todayProducts, recentTransactions] =
          await Promise.all([
            paymentsCollection
              .aggregate([
                {
      ...
      case "attendance": {
        if (!staffId) {
          responseData = { error: "Staff ID required" };
          break;
        }
        const [recentAttendance, totalStats] = await Promise.all([
          attendanceCollection
      ...
      case "monthly-trends": {
        const [monthSales, lastMonthSales, monthProducts, dailyTrend] =
          await Promise.all([
            paymentsCollection
              .aggregate([
                {
      ...
      case "all-time-stats": {
        ...
        responseData = {
          ...
          generatedAt: new Date().toISOString(),
          totalRevenue:
            Math.round((allTimeSales[0]?.totalRevenue || 0) * 100) / 100,
          totalTransactions: allTimeSales[0]?.totalTransactions || 0,
          topProductsAllTime: allTimeProducts,
          totalProducts,
          totalCustomers,
      ...
      case "inventory-insights": {
        ...
        responseData = {
          ...
          totalProducts,
          totalInventoryValue:
            Math.round((totalValue[0]?.totalValue || 0) * 100) / 100,
          lowStockItems,
          criticalStockItems,
          itemsNeedingRestock: lowStockItems.length + criticalStockItems.length,
        };
      ...
      case "full-summary": {
        ...
        const monthStartFull = new Date(
          nowFull.setMonth(nowFull.getMonth() - 1),
        );

        const [todaySales, monthSales, allTimeSales, inventoryStats] =
          await Promise.all([
            paymentsCollection
              .aggregate([
                {
                  $match: {
                    createdAt: { $gte: todayStartFull },
                    ...(staffId && { cashierId: staffId }),
                  },
                },
                {
        ...
        responseData = {
          ...
          today: {
            todayRevenue:
              Math.round((todaySales[0]?.totalRevenue || 0) * 100) / 100,
            todayTransactions: todaySales[0]?.totalTransactions || 0,
          },
          month: {
            monthRevenue:
              Math.round((monthSales[0]?.totalRevenue || 0) * 100) / 100,
            monthTransactions: monthSales[0]?.totalTransactions || 0,
          },
          allTime: {
            totalRevenue:
