...
export async function POST(request: NextRequest) {
  try {
    ...
    const existing = await MONGODB.collection<Inventory>(INVENTORY_COLLECTION).findOne({
    ...
    const result = await MONGODB.collection(INVENTORY_COLLECTION).insertOne(newItem);
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (updates.name && updates.name.toLowerCase() !== existing.name.toLowerCase()) {
      const nameExists = await MONGODB.collection<Inventory>(INVENTORY_COLLECTION).findOne({
    ...
    const updated = await MONGODB.collection<Inventory>(INVENTORY_COLLECTION).findOne({
      _id: new ObjectId(id)
    });
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const hasAdjustments = await MONGODB.collection('stockAdjustments')
    ...
    const result = await MONGODB.collection(INVENTORY_COLLECTION).deleteOne({
      _id: new ObjectId(id)
    });
