...
async function handleResponse<T>(response: Response): Promise<T> {
  console.log(
    `[InventoryService] FETCH ${response.url} - Status: ${response.status}`,
  );
  ...
  try {
    ...
    if (
      response.url.includes("/api/products/stocks") &&
      !response.url.includes("/adjust")
    ) {
...
export async function getInventoryByNames(
  names: string[],
): Promise<Inventory[]> {
  try {
