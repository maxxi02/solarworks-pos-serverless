  ...
  for (const line of lines) {
    switch (line.type) {
      ...
      case 'text': {
        ...
        if (line.doubleSize || line.doubleHeight) chunks.push(...CMD.NORMAL_SIZE);
      ...
      case 'two-col': {
        chunks.push(...CMD.ALIGN_LEFT);
        if (line.bold) chunks.push(...CMD.BOLD_ON);
        const row = twoColumnLine(line.left ?? '', line.right ?? '', LINE_WIDTH);
...
export type ReceiptLine =
  | { type: 'init' }
  | { type: 'text'; text: string; align?: 'left' | 'center' | 'right'; bold?: boolean; doubleHeight?: boolean; doubleSize?: boolean }
...
export function buildCustomerReceiptLines(data: ReceiptBuildInput): ReceiptLine[] {
  const lines: ReceiptLine[] = [];
  const fmt = (n: number) => `P${n.toFixed(2)}`;

  // Header
  lines.push({ type: 'text', text: data.businessName, align: 'center', bold: true, doubleHeight: true });
  if (data.businessAddress) lines.push({ type: 'text', text: data.businessAddress, align: 'center' });
  if (data.businessPhone) lines.push({ type: 'text', text: data.businessPhone, align: 'center' });
  lines.push({ type: 'divider', char: '=' });

  // Order info
  lines.push({ type: 'two-col', left: 'Order #:', right: data.orderNumber });
  lines.push({ type: 'two-col', left: 'Date:', right: new Date(data.timestamp).toLocaleDateString() });
  lines.push({ type: 'two-col', left: 'Time:', right: new Date(data.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) });
  lines.push({ type: 'two-col', left: 'Cashier:', right: data.cashier });
  lines.push({ type: 'two-col', left: 'Customer:', right: data.customerName || 'Walk-in' });
  lines.push({ type: 'two-col', left: 'Type:', right: data.orderType === 'dine-in' ? 'Dine In' : 'Take Away' });
  if (data.tableNumber) lines.push({ type: 'two-col', left: 'Table:', right: data.tableNumber });
  if (data.orderNote) lines.push({ type: 'text', text: `Note: ${data.orderNote}` });
  lines.push({ type: 'divider' });

  // Items header
  lines.push({ type: 'text', text: 'ITEM                  QTY   AMT', bold: true });
  lines.push({ type: 'divider' });
  ...
  if (data.discountTotal > 0) {
    lines.push({ type: 'two-col', left: 'Discount:', right: `-${fmt(data.discountTotal)}` });
  }
  lines.push({ type: 'divider' });
  ...
  // Payment
  lines.push({ type: 'two-col', left: 'Payment:', right: data.paymentMethod.toUpperCase() });
  ...
  if (data.paymentMethod === 'cash' && data.amountPaid !== undefined) {
    lines.push({ type: 'two-col', left: 'Tendered:', right: fmt(data.amountPaid) });
  ...
  if (data.seniorPwdIds?.length) {
    ...
    data.seniorPwdIds.forEach(id => lines.push({ type: 'text', text: `  ${id}` }));
  }

  lines.push({ type: 'divider', char: '=' });

  // Footer
  if (data.receiptMessage) lines.push({ type: 'text', text: data.receiptMessage, align: 'center' });
  lines.push({ type: 'text', text: 'Thank you for dining with us!', align: 'center' });
  lines.push({ type: 'text', text: `[${data.orderNumber}]`, align: 'center' });

  if (data.isReprint) {
    lines.push({ type: 'divider' });
    lines.push({ type: 'text', text: '*** REPRINT ***', align: 'center', bold: true });
  }
...
export function buildKitchenOrderLines(data: ReceiptBuildInput): ReceiptLine[] {
  ...
  lines.push({ type: 'text', text: '*** KITCHEN ORDER ***', align: 'center', bold: true, doubleHeight: true });
  lines.push({ type: 'divider', char: '=' });

  lines.push({ type: 'two-col', left: 'Order #:', right: data.orderNumber, bold: true });
  lines.push({ type: 'two-col', left: 'Time:', right: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) });
  lines.push({ type: 'two-col', left: 'Type:', right: data.orderType === 'dine-in' ? 'DINE IN' : 'TAKE AWAY' });
  if (data.tableNumber) lines.push({ type: 'two-col', left: 'Table:', right: data.tableNumber, bold: true });
  if (data.customerName) lines.push({ type: 'two-col', left: 'Customer:', right: data.customerName });

  lines.push({ type: 'divider', char: '=' });

  // All items for kitchen
  for (const item of data.items) {
    lines.push({ type: 'text', text: `${item.quantity}x  ${item.name}`, bold: true, doubleHeight: true });
  }
  ...
  lines.push({ type: 'text', text: '*** PLEASE PREPARE ***', align: 'center', bold: true });
  lines.push({ type: 'cut' });
