...
function toReceiptBuildInput(receipt: ReceiptData, settings: ReceiptSettings): ReceiptBuildInput {
  return {
...
export async function printReceipt(
  ...
  if (settings.printReceipt) {
    if (printerManager.getStatus().usb === 'connected' || printerManager.getStatus().usb === 'printing') {
  ...
  if (settings.kitchenPrinter?.enabled && hasItems) {
    if (printerManager.getStatus().bluetooth === 'connected' || printerManager.getStatus().bluetooth === 'printing') {
...
function buildReceiptText(receipt: ReceiptData, settings: ReceiptSettings): string {
  const W = 32;
  ...
  s += lr('Type:', receipt.orderType === 'dine-in' ? 'Dine In' : 'Take Away') + '\n';
  if (receipt.tableNumber) s += lr('Table:', receipt.tableNumber) + '\n';
  s += div + '\n';
  receipt.items.forEach(item => {
    const p = item.hasDiscount ? item.price * 0.8 : item.price;
    const tot = (p * item.quantity).toFixed(2);
    s += lr(`${item.quantity}x ${item.name.substring(0, 22)}`, `P${tot}`) + '\n';
    if (item.hasDiscount) s += '  [Senior/PWD 20% off]\n';
  });
  s += div + '\n';
  s += lr('Subtotal:', fmt(receipt.subtotal)) + '\n';
  if (receipt.discountTotal > 0) s += lr('Discount:', `-${fmt(receipt.discountTotal)}`) + '\n';
...
function buildKitchenText(receipt: ReceiptData): string {
  ...
  s += dbDiv + '\n';
  s += lr('Order #:', receipt.orderNumber) + '\n';
  s += lr('Time:', new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })) + '\n';
  s += lr('Type:', receipt.orderType === 'dine-in' ? 'DINE IN' : 'TAKE AWAY') + '\n';
  if (receipt.tableNumber) s += lr('Table:', receipt.tableNumber) + '\n';
